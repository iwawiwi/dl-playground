...
class TeacherTrainingModule(LightningModule):
    """
    ...
    def __init__(
        self,
        teacher_model_name: str = "densenet121",
        image_size: int = 244,
        num_classes: int = 10,
        lr: float = 0.001,
        ...
        # model
        self.model = self.load_model(self.hparams.teacher_model_name)
...
class StudentTrainingModule(LightningModule):
    ...
    def __init__(
        ...
        student_model_name: str = "densenet121",
        image_size: int = 244,
        num_classes: int = 10,
        lr: float = 0.001,
        weight_decay: float = 0.0005,
        ...
        # model
        self.model = self.load_model(self.hparams.student_model_name)
        # load teacher model
        self.teacher = TeacherTrainingModule.load_from_checkpoint(teacher_model_path).model
    ...
    def training_step(self, batch, batch_idx):
        ...
        teacher_pred = self.teacher.forward(images)  # teacher prediction
        ...
        acc = self.train_acc(
            student_pred, labels
        )  # accuracy of student based on true target label
        self.log("train/loss", loss, on_step=False, on_epoch=True, prog_bar=False)
        self.log("train/acc", acc, on_step=False, on_epoch=True, prog_bar=True)
