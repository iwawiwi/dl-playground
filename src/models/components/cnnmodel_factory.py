...
class CnnClassifier(nn.Module):
    ...
    def get_fclayer_list(self, hidden_layers):
        ...
        layers.append(("fc_out", nn.Linear(output_layers[-1], self.num_classes)))
        return layers
...
class SimpleClassifier(CnnClassifier):
    ...
    def __init__(
        ...
        self.fc1 = nn.Linear(
            64 * 28 * 28, 512
        )  # it is supposed that image_size parameter to calculate input of linear layer
...
class Resnet18Classifier(CnnClassifier):
    ...
    def __init__(
        ...
        self.model.fc = nn.Sequential(
            OrderedDict(self.get_fclayer_list([512, 100]))
        )  # resnet18 final classification layer are modified
...
class Vgg16Classifier(CnnClassifier):
    ...
    def __init__(
        ...
        self.model.classifier = nn.Sequential(
            OrderedDict(self.get_fclayer_list([512 * 7 * 7, 4096, 4096, 2048, 512]))
        )  # vgg16 final classification layer are modified
...
class DenseNet121Classifier(CnnClassifier):
    ...
    def __init__(
        ...
        self.model.classifier = nn.Sequential(
            OrderedDict(self.get_fclayer_list([1024, 500]))
        )  # densenet121 final classification layer are modified
